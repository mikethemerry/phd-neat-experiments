...
x3_inputs = create_n_points(400, 2, -1, 1)

x3_outputs = [
    tuple( [x3(tup[0], tup[1])] ) for tup in x3_inputs
]
...
def eval_genomes(genome, config):
    ...
    error = 400.0
    net = neat.nn.FeedForwardNetwork.create(genome, config)
    for xi, xo in zip(x3_inputs, x3_outputs):
...
def run(config_file, runNumber):
    ...
    try:
        ...
        pass
    p.add_reporter(neat.Checkpointer(25, filename_prefix='./results/run-%s/x3-checkpoint-' % runNumber))
